...
## 2025-04-20 - Mapping Standard Keyboard Shortcuts to Custom Interactions
**Learning:** Users expect standard OS shortcuts like 'Ctrl+C' to work even for custom interactive elements. Mapping these to custom "Copy-to-Clipboard" behaviors reduces cognitive load and makes the web application feel more integrated with the user's workflow.
**Action:** Always consider mapping common keyboard shortcuts to related custom UI actions.

## 2025-04-21 - Accessible Search Reset Announcement
**Learning:** When a search filter is cleared (via a button or the 'Escape' key), screen reader users may not receive immediate feedback that the results have been reset. Manually updating an ARIA live region with a "Search cleared" message provides essential confirmation of the state change.
**Action:** Always pair search reset actions with an explicit accessibility announcement to ensure parity between visual and screen reader feedback.
